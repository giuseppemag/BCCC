...
type AfterMany<l, a> =
  l extends [infer f, ...infer fs] ? 
    f extends keyof BaseFunctors<a> ? { res:AfterMany<fs, BaseFunctors<a>[f]> }
  : never
  : a

interface FlattenRes<a>
  { 
    v1:a extends { res:infer b } ? FlattenRes<b> : never
    v2:a
  }

type T1 = AfterMany<["Option", "List", "List"], number>["res"]["res"]["res"] // == List<List<Option<number>>>
